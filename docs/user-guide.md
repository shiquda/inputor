...
调试页面向高级用户，实时滚动显示 inputor 捕获到的输入事件流。调试记录只缓存在内存中，不会写入统计文件，也不会被导出。
...
如果你想迁移或归档完整统计数据，可在设置页使用「备份当前统计源」生成 JSON 备份，或填写「统计源路径」后切换到其他 JSON 统计文件。
